...
export const chat_code_completion_instructions = (
  ...
  `Find correct replacement text for the <missing_text> symbol. Correctly formatted response begins with a code block containing the replacement text and then proceeds with explanation. In the code block place the following text as a first-line comment: "${file_path} ${
    row + 1
  }:${
    column + 1
