  ...
  const show_quick_pick = async (): Promise<boolean> => {
    const items: vscode.QuickPickItem[] = [
      ...
      },
      {
        ...
        detail: config_to_edit.instructions_placement ?? 'before-all'
      }
    ]
    ...
    switch (selected_option) {
      ...
      case 'Model': {
        ...
        try {
          ...
          Logger.error({
            function_name: 'handle_edit_edit_context_configuration case Model',
      ...
      case 'Instructions Placement': {
        ...
        if (selected_placement !== undefined) {
          updated_config.instructions_placement = selected_placement as any
        } else {
          return await show_quick_pick()
        }
        break
