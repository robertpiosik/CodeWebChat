...
- **Context, once polluted, won't recover.** Whenever the model is not getting your intent—adjust included files, instructions and start afresh. Telling the model it is wrong gets you apologies that are in most cases just a trained-in, dull text.
- **Agents are expensive and can fail you.** Pattern-matching technology backinig them is sufficient and encouraging as you start, but with ever-increasing complexity of real-world projects you may get stuck in unseen in training datasets territories. Your bills can skyrocket when you happen to run in circles.
