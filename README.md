...
Code Web Chat (CWC) is a free and open-source, privacy-first **AI coding toolkit for VS Code**. It helps construct zero-overhead XML-formatted prompts for fast edits using:

- chatbots—ChatGPT, Claude, Gemini, etc.
- APIs—remote and local model providers

...and apply suggested code with automatic checkpoints.
...
First-class support for [prompt caching](https://developers.openai.com/api/docs/guides/prompt-caching/) by ordering the selected files by update recency. Make your chatbot quotas last longer or lower token costs by up to 90% when calling APIs. Minimize latency of self-hosted models.
