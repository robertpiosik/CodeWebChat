...
Because it's not possible to train a model to keep architectural order in the repository (interconnectedness of things are unique patterns), we believe that ticking ✓ folders and files on a file tree is an essential activity when interacting with LLMs. It also makes simple prompts just work!

Generated XML-formatted prompts are zero-overhead (click an example to expand):
