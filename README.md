...
- **Programming is about building mental models.** Being close to the codebase makes context selection efortless and AI outputs predictable.
- **Single-turn interaction with the model save time.** There is no orchiestrated planning or file lookups for automated context collection—a single response from the model contains all the changes for the task.
