...
Code Web Chat is **designed from the ground-up for [prompt caching](https://developers.openai.com/api/docs/guides/prompt-caching) across tasks** via smart context files ordering. Make the most of your usage limits. When calling APIs—reduce latency by up to 80% and input token costs by up to 90%.
