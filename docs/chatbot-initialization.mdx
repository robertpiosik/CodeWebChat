...
```
Find correct replacement text for the <missing_text> symbol. Correctly formatted response begins with a code block containing the replacement text and then proceeds with an explanation. Always refer to symbol "<missing_text>" as "cursor position" and "replacement" as "completion".
<files>
  <text title="...">...</text> // selected websites
  ...
