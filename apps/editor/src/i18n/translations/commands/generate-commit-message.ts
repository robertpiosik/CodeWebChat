export const translations = {
  'command.generate-commit-message.input.title': {
    en: 'Commit Message',
    pl: 'Wiadomość commita',
    'zh-cn': '提交信息',
    ja: 'コミットメッセージ',
    'zh-tw': '提交訊息',
    de: 'Commit-Nachricht',
    es: 'Mensaje de commit',
    fr: 'Message de commit',
    'pt-br': 'Mensagem de commit',
    ru: 'Сообщение коммита',
    ko: '커밋 메시지',
    it: 'Messaggio di commit',
    tr: 'İşleme (Commit) Mesajı',
    cs: 'Zpráva commitu',
    hu: 'Commit üzenet',
    bg: 'Commit съобщение'
  },
  'command.generate-commit-message.input.prompt': {
    en: 'Edit the commit message.',
    pl: 'Edytuj wiadomość commita.',
    'zh-cn': '编辑提交信息。',
    ja: 'コミットメッセージを編集します。',
    'zh-tw': '編輯提交訊息。',
    de: 'Bearbeiten Sie die Commit-Nachricht.',
    es: 'Edita el mensaje de commit.',
    fr: 'Modifiez le message de commit.',
    'pt-br': 'Edite a mensagem de commit.',
    ru: 'Отредактируйте сообщение коммита.',
    ko: '커밋 메시지를 편집합니다.',
    it: 'Modifica il messaggio di commit.',
    tr: 'İşleme (commit) mesajını düzenleyin.',
    cs: 'Upravte zprávu commitu.',
    hu: 'Szerkessze a commit üzenetet.',
    bg: 'Редактирайте commit съобщението.'
  },
  'command.generate-commit-message.input.accept': {
    en: 'Accept',
    pl: 'Akceptuj',
    'zh-cn': '接受',
    ja: '承認',
    'zh-tw': '接受',
    de: 'Akzeptieren',
    es: 'Aceptar',
    fr: 'Accepter',
    'pt-br': 'Aceitar',
    ru: 'Принять',
    ko: '수락',
    it: 'Accetta',
    tr: 'Kabul Et',
    cs: 'Přijmout',
    hu: 'Elfogadás',
    bg: 'Приемане'
  },
  'command.generate-commit-message.copied': {
    en: 'Commit message prompt copied to clipboard ({tokens} tokens).',
    pl: 'Prompt wiadomości commita skopiowano do schowka ({tokens} tokenów).',
    'zh-cn': '提交信息提示已复制到剪贴板 ({tokens} 个 token)。',
    ja: 'コミットメッセージのプロンプトをクリップボードにコピーしました ({tokens} トークン)。',
    'zh-tw': '提交訊息提示已複製到剪貼簿 ({tokens} 個 token)。',
    de: 'Commit-Nachricht-Prompt in die Zwischenablage kopiert ({tokens} Tokens).',
    es: 'Prompt del mensaje de commit copiado al portapapeles ({tokens} tokens).',
    fr: 'Prompt de message de commit copié dans le presse-papiers ({tokens} jetons).',
    'pt-br':
      'Prompt da mensagem de commit copiado para a área de transferência ({tokens} tokens).',
    ru: 'Промпт сообщения коммита скопирован в буфер обмена ({tokens} токенов).',
    ko: '커밋 메시지 프롬프트가 클립보드에 복사되었습니다 ({tokens} 토큰).',
    it: 'Prompt del messaggio di commit copiato negli appunti ({tokens} token).',
    tr: 'İşleme mesajı istemi panoya kopyalandı ({tokens} token).',
    cs: 'Prompt zprávy commitu zkopírován do schránky ({tokens} tokenů).',
    hu: 'Commit üzenet prompt a vágólapra másolva ({tokens} token).',
    bg: 'Промптът за commit съобщение е копиран в клипборда ({tokens} токена).'
  },
  'command.generate-commit-message.progress.title': {
    en: 'Commit Changes',
    pl: 'Zatwierdź zmiany',
    'zh-cn': '提交更改',
    ja: '変更をコミット',
    'zh-tw': '提交變更',
    de: 'Änderungen committen',
    es: 'Confirmar cambios',
    fr: 'Valider les modifications',
    'pt-br': 'Confirmar alterações',
    ru: 'Зафиксировать изменения',
    ko: '변경 사항 커밋',
    it: 'Commit delle modifiche',
    tr: 'Değişiklikleri işle',
    cs: 'Potvrdit změny',
    hu: 'Változások véglegesítése',
    bg: 'Потвърждаване на промените'
  },
  'command.generate-commit-message.select-repository': {
    en: 'Select a repository to generate the commit message for',
    pl: 'Wybierz repozytorium, dla którego chcesz wygenerować wiadomość commita',
    'zh-cn': '选择要生成提交信息的仓库',
    ja: 'コミットメッセージを生成するリポジトリを選択してください',
    'zh-tw': '選擇要產生提交訊息的儲存庫',
    de: 'Wählen Sie ein Repository aus, für das die Commit-Nachricht generiert werden soll',
    es: 'Seleccione un repositorio para generar el mensaje de commit',
    fr: 'Sélectionnez un dépôt pour générer le message de commit',
    'pt-br': 'Selecione um repositório para gerar a mensagem de commit',
    ru: 'Выберите репозиторий для генерации сообщения коммита',
    ko: '커밋 메시지를 생성할 저장소 선택',
    it: 'Seleziona un repository per generare il messaggio di commit',
    tr: 'İşleme mesajı oluşturmak için bir depo seçin',
    cs: 'Vyberte repozitář pro vygenerování zprávy commitu',
    hu: 'Válasszon egy tárolót a commit üzenet generálásához',
    bg: 'Изберете хранилище за генериране на commit съобщение'
  },
  'command.generate-commit-message.unstaged-files': {
    en: 'Unstaged Files',
    pl: 'Nieprzygotowane pliki',
    'zh-cn': '未暂存的文件',
    ja: 'ステージされていないファイル',
    'zh-tw': '未暫存的檔案',
    de: 'Nicht bereitgestellte Dateien',
    es: 'Archivos no preparados',
    fr: 'Fichiers non indexés',
    'pt-br': 'Arquivos não preparados',
    ru: 'Неиндексированные файлы',
    ko: '스테이징되지 않은 파일',
    it: 'File non in stage',
    tr: 'Hazırlanmamış (Unstaged) Dosyalar',
    cs: 'Nepřipravené soubory',
    hu: 'Nem előkészített fájlok',
    bg: 'Неиндексирани файлове'
  },
  'command.generate-commit-message.select-files': {
    en: 'Select files to commit',
    pl: 'Wybierz pliki do commita',
    'zh-cn': '选择要提交的文件',
    ja: 'コミットするファイルを選択してください',
    'zh-tw': '選擇要提交的檔案',
    de: 'Wählen Sie Dateien zum Committen aus',
    es: 'Selecciona los archivos para el commit',
    fr: 'Sélectionnez les fichiers à valider',
    'pt-br': 'Selecione os arquivos para o commit',
    ru: 'Выберите файлы для коммита',
    ko: '커밋할 파일 선택',
    it: 'Seleziona i file per il commit',
    tr: 'İşlenecek dosyaları seçin',
    cs: 'Vyberte soubory pro commit',
    hu: 'Válassza ki a commitálandó fájlokat',
    bg: 'Изберете файлове за commit'
  },
  'command.generate-commit-message.show-diff': {
    en: 'Show Diff',
    pl: 'Pokaż zmiany',
    'zh-cn': '显示差异',
    ja: '変更を表示',
    'zh-tw': '顯示差異',
    de: 'Änderungen anzeigen',
    es: 'Mostrar cambios',
    fr: 'Afficher les modifications',
    'pt-br': 'Mostrar alterações',
    ru: 'Показать изменения',
    ko: '차이 보기',
    it: 'Mostra differenze',
    tr: 'Değişiklikleri Göster',
    cs: 'Zobrazit změny',
    hu: 'Eltérések megjelenítése',
    bg: 'Покажи разликите'
  },
  'command.generate-commit-message.attach-ascii-tree.attach': {
    en: 'Attach',
    pl: 'Dołącz',
    'zh-cn': '附加',
    ja: '添付する',
    'zh-tw': '附加',
    de: 'Anhängen',
    es: 'Adjuntar',
    fr: 'Joindre',
    'pt-br': 'Anexar',
    ru: 'Прикрепить',
    ko: '첨부',
    it: 'Allega',
    tr: 'Ekle',
    cs: 'Připojit',
    hu: 'Csatolás',
    bg: 'Прикачи'
  },
  'command.generate-commit-message.attach-ascii-tree.skip': {
    en: 'Skip',
    pl: 'Pomiń',
    'zh-cn': '跳过',
    ja: 'スキップ',
    'zh-tw': '跳過',
    de: 'Überspringen',
    es: 'Omitir',
    fr: 'Ignorer',
    'pt-br': 'Ignorar',
    ru: 'Пропустить',
    ko: '건너뛰기',
    it: 'Salta',
    tr: 'Atla',
    cs: 'Přeskočit',
    hu: 'Kihagyás',
    bg: 'Пропусни'
  },
  'command.generate-commit-message.attach-ascii-tree.title': {
    en: 'ASCII File Tree',
    pl: 'Drzewo plików ASCII',
    'zh-cn': 'ASCII 文件树',
    ja: 'ASCII ファイルツリー',
    'zh-tw': 'ASCII 檔案樹',
    de: 'ASCII-Dateibaum',
    es: 'Árbol de archivos ASCII',
    fr: 'Arborescence de fichiers ASCII',
    'pt-br': 'Árvore de Arquivos ASCII',
    ru: 'ASCII дерево файлов',
    ko: 'ASCII 파일 트리',
    it: 'Albero dei file ASCII',
    tr: 'ASCII Dosya Ağacı',
    cs: 'Strom souborů ASCII',
    hu: 'ASCII fájlfa',
    bg: 'ASCII дърво на файловете'
  },
  'command.generate-commit-message.attach-ascii-tree.placeholder': {
    en: 'Attach an ASCII file tree of the context files to the commit message?',
    pl: 'Dołączyć drzewo plików ASCII plików kontekstowych do wiadomości commita?',
    'zh-cn': '是否将上下文文件的 ASCII 文件树附加到提交信息中？',
    ja: 'コンテキストファイルの ASCII ファイルツリーをコミットメッセージに添付しますか？',
    'zh-tw': '是否將上下文檔案的 ASCII 檔案樹附加到提交訊息中？',
    de: 'Möchten Sie einen ASCII-Dateibaum der Kontextdateien an die Commit-Nachricht anhängen?',
    es: '¿Adjuntar un árbol de archivos ASCII de los archivos de contexto al mensaje de commit?',
    fr: 'Joindre une arborescence de fichiers ASCII des fichiers de contexte au message de commit ?',
    'pt-br':
      'Anexar uma árvore de arquivos ASCII dos arquivos de contexto à mensagem de commit?',
    ru: 'Прикрепить ASCII дерево контекстных файлов к сообщению коммита?',
    ko: '컨텍스트 파일의 ASCII 파일 트리를 커밋 메시지에 첨부하시겠습니까?',
    it: 'Allegare un albero dei file ASCII dei file di contesto al messaggio di commit?',
    tr: 'Bağlam dosyalarının ASCII dosya ağacı işleme mesajına eklensin mi?',
    cs: 'Připojit strom souborů ASCII kontextových souborů ke zprávě commitu?',
    hu: 'Csatoljon egy ASCII fájlfát a kontextusfájlokról a commit üzenethez?',
    bg: 'Да се прикачи ли ASCII дърво на контекстните файлове към commit съобщението?'
  },
  'command.generate-commit-message.action-quick-pick.title': {
    en: 'Commit Message Actions',
    pl: 'Akcje wiadomości commita',
    'zh-cn': '提交信息操作',
    ja: 'コミットメッセージのアクション',
    'zh-tw': '提交訊息動作',
    de: 'Commit-Nachrichten-Aktionen',
    es: 'Acciones de mensaje de commit',
    fr: 'Actions de message de commit',
    'pt-br': 'Ações da mensagem de commit',
    ru: 'Действия с сообщением коммита',
    ko: '커밋 메시지 작업',
    it: 'Azioni del messaggio di commit',
    tr: 'İşleme Mesajı Eylemleri',
    cs: 'Akce zprávy commitu',
    hu: 'Commit üzenet műveletek',
    bg: 'Действия с commit съобщение'
  },
  'command.generate-commit-message.action-quick-pick.placeholder': {
    en: 'Choose how to proceed with {tokens} tokens',
    pl: 'Wybierz, jak kontynuować ({tokens} tokenów)',
    'zh-cn': '选择如何继续（{tokens} 个 token）',
    ja: '続行方法を選択してください（{tokens} トークン）',
    'zh-tw': '選擇如何繼續（{tokens} 個 token）',
    de: 'Wählen Sie, wie Sie fortfahren möchten ({tokens} Tokens)',
    es: 'Elige cómo proceder ({tokens} tokens)',
    fr: 'Choisissez comment procéder ({tokens} jetons)',
    'pt-br': 'Escolha como prosseguir ({tokens} tokens)',
    ru: 'Выберите, как продолжить ({tokens} токенов)',
    ko: '진행 방법 선택 ({tokens} 토큰)',
    it: 'Scegli come procedere ({tokens} token)',
    tr: 'Nasıl devam edeceğinizi seçin ({tokens} token)',
    cs: 'Vyberte, jak pokračovat ({tokens} tokenů)',
    hu: 'Válassza ki a folytatás módját ({tokens} token)',
    bg: 'Изберете как да продължите ({tokens} токена)'
  },
  'command.generate-commit-message.action.autofill-in-chatbot': {
    en: 'Autofill in chatbot',
    pl: 'Autouzupełnij w czatbocie',
    'zh-cn': '在聊天机器人中自动填充',
    ja: 'チャットボットで自動入力',
    'zh-tw': '在聊天機器人中自動填寫',
    de: 'In Chatbot automatisch ausfüllen',
    es: 'Autocompletar en el chatbot',
    fr: 'Remplissage automatique dans le chatbot',
    'pt-br': 'Preenchimento automático no chatbot',
    ru: 'Автозаполнение в чат-боте',
    ko: '챗봇에 자동 입력',
    it: 'Compilazione automatica nel chatbot',
    tr: 'Sohbet robotunda otomatik doldur',
    cs: 'Automaticky vyplnit v chatbotu',
    hu: 'Automatikus kitöltés a chatbotban',
    bg: 'Автоматично попълване в чатбота'
  },
  'command.generate-commit-message.action.make-api-call': {
    en: 'Make API call',
    pl: 'Wywołaj API',
    'zh-cn': '调用 API',
    ja: 'API を呼び出す',
    'zh-tw': '呼叫 API',
    de: 'API-Aufruf durchführen',
    es: 'Hacer llamada API',
    fr: 'Faire un appel API',
    'pt-br': 'Fazer chamada de API',
    ru: 'Выполнить API-запрос',
    ko: 'API 호출',
    it: 'Effettua chiamata API',
    tr: 'API çağrısı yap',
    cs: 'Provést API volání',
    hu: 'API hívás',
    bg: 'Извикване на API'
  },
  'command.generate-commit-message.action.enter-manually': {
    en: 'Enter manually',
    pl: 'Wpisz ręcznie',
    'zh-cn': '手动输入',
    ja: '手動で入力',
    'zh-tw': '手動輸入',
    de: 'Manuell eingeben',
    es: 'Ingresar manualmente',
    fr: 'Saisir manuellement',
    'pt-br': 'Inserir manualmente',
    ru: 'Ввести вручную',
    ko: '수동 입력',
    it: 'Inserisci manualmente',
    tr: 'Manuel gir',
    cs: 'Zadat ručně',
    hu: 'Kézi megadás',
    bg: 'Въведете ръчно'
  },
  'command.generate-commit-message.action.copy-prompt': {
    en: 'Copy prompt',
    pl: 'Skopiuj prompt',
    'zh-cn': '复制提示',
    ja: 'プロンプトをコピー',
    'zh-tw': '複製提示',
    de: 'Prompt kopieren',
    es: 'Copiar prompt',
    fr: 'Copier le prompt',
    'pt-br': 'Copiar prompt',
    ru: 'Скопировать промпт',
    ko: '프롬프트 복사',
    it: 'Copia prompt',
    tr: 'İstemi kopyala',
    cs: 'Kopírovat prompt',
    hu: 'Prompt másolása',
    bg: 'Копиране на промпт'
  },
  'command.generate-commit-message.disabled-during-preview': {
    en: 'Commit message generation is disabled during response preview.',
    pl: 'Generowanie wiadomości commita jest wyłączone podczas podglądu odpowiedzi.',
    'zh-cn': '在响应预览期间，生成提交信息功能已禁用。',
    ja: '応答プレビュー中は、コミットメッセージの生成が無効になります。',
    'zh-tw': '在回應預覽期間，產生提交訊息功能已停用。',
    de: 'Die Erstellung von Commit-Nachrichten ist während der Antwortvorschau deaktiviert.',
    es: 'La generación de mensajes de confirmación está deshabilitada durante la vista previa de la respuesta.',
    fr: "La génération de message de commit est désactivée pendant l'aperçu de la réponse.",
    'pt-br':
      'A geração de mensagem de commit está desativada durante a visualização da resposta.',
    ru: 'Генерация сообщения коммита отключена во время предпросмотра ответа.',
    ko: '응답 미리보기 중에는 커밋 메시지 생성이 비활성화됩니다.',
    it: "La generazione del messaggio di commit è disabilitata durante l'anteprima della risposta.",
    tr: 'Yanıt önizlemesi sırasında işleme mesajı oluşturma devre dışı bırakılır.',
    cs: 'Generování zprávy commitu je během náhledu odpovědi zakázáno.',
    hu: 'A commit üzenet generálása a válasz előnézete alatt le van tiltva.',
    bg: 'Генерирането на commit съобщение е деактивирано по време на преглед на отговора.'
  },
  'command.generate-commit-message.error.empty-response': {
    en: 'API request returned an empty response.',
    pl: 'Żądanie API zwróciło pustą odpowiedź.',
    'zh-cn': 'API 请求返回了空响应。',
    ja: 'API リクエストが空のレスポンスを返しました。',
    'zh-tw': 'API 請求回傳了空回應。',
    de: 'API-Anfrage gab eine leere Antwort zurück.',
    es: 'La solicitud API devolvió una respuesta vacía.',
    fr: 'La requête API a renvoyé une réponse vide.',
    'pt-br': 'A solicitação de API retornou uma resposta vazia.',
    ru: 'API-запрос вернул пустой ответ.',
    ko: 'API 요청이 빈 응답을 반환했습니다.',
    it: 'La richiesta API ha restituito una risposta vuota.',
    tr: 'API isteği boş bir yanıt döndürdü.',
    cs: 'Požadavek API vrátil prázdnou odpověď.',
    hu: 'Az API kérés üres választ adott vissza.',
    bg: 'API заявката върна празен отговор.'
  },
  'command.generate-commit-message.attach-context-files.attach': {
    en: 'Use',
    pl: 'Użyj',
    'zh-cn': '使用',
    ja: '使用する',
    'zh-tw': '使用',
    de: 'Verwenden',
    es: 'Usar',
    fr: 'Utiliser',
    'pt-br': 'Usar',
    ru: 'Использовать',
    ko: '사용',
    it: 'Usa',
    tr: 'Kullan',
    cs: 'Použít',
    hu: 'Használat',
    bg: 'Използвай'
  },
  'command.generate-commit-message.attach-context-files.skip': {
    en: 'Skip',
    pl: 'Pomiń',
    'zh-cn': '跳过',
    ja: 'スキップ',
    'zh-tw': '跳過',
    de: 'Überspringen',
    es: 'Omitir',
    fr: 'Ignorer',
    'pt-br': 'Ignorar',
    ru: 'Пропустить',
    ko: '건너뛰기',
    it: 'Salta',
    tr: 'Atla',
    cs: 'Přeskočit',
    hu: 'Kihagyás',
    bg: 'Пропусни'
  },
  'command.generate-commit-message.attach-context-files.title': {
    en: 'Context Files',
    pl: 'Pliki kontekstowe',
    'zh-cn': '上下文文件',
    ja: 'コンテキストファイル',
    'zh-tw': '上下文檔案',
    de: 'Kontextdateien',
    es: 'Archivos de contexto',
    fr: 'Fichiers de contexte',
    'pt-br': 'Arquivos de contexto',
    ru: 'Контекстные файлы',
    ko: '컨텍스트 파일',
    it: 'File di contesto',
    tr: 'Bağlam Dosyaları',
    cs: 'Kontextové soubory',
    hu: 'Kontextusfájlok',
    bg: 'Контекстни файлове'
  },
  'command.generate-commit-message.attach-context-files.placeholder': {
    en: 'Should use context files to improve accuracy?',
    pl: 'Czy użyć plików kontekstowych, aby poprawić dokładność?',
    'zh-cn': '是否使用上下文文件以提高准确性？',
    ja: '精度を向上させるためにコンテキストファイルを使用しますか？',
    'zh-tw': '是否使用上下文檔案以提高準確性？',
    de: 'Sollten Kontextdateien verwendet werden, um die Genauigkeit zu verbessern?',
    es: '¿Debería usar archivos de contexto para mejorar la precisión?',
    fr: 'Faut-il utiliser des fichiers de contexte pour améliorer la précision ?',
    'pt-br': 'Deve usar arquivos de contexto para melhorar a precisão?',
    ru: 'Использовать ли контекстные файлы для повышения точности?',
    ko: '정확도를 높이기 위해 컨텍스트 파일을 사용하시겠습니까?',
    it: 'Usare i file di contesto per migliorare la precisione?',
    tr: 'Doğruluğu artırmak için bağlam dosyaları kullanılsın mı?',
    cs: 'Měly by se použít kontextové soubory pro zlepšení přesnosti?',
    hu: 'Használjon kontextusfájlokat a pontosság javításához?',
    bg: 'Трябва ли да се използват контекстни файлове за подобряване на точността?'
  }
} as const
