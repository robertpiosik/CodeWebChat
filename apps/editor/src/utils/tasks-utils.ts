...
export const get_tasks_file_path = (params: {
...
export const save_all_tasks = (params: {
  context: vscode.ExtensionContext
  ...
  const file_path = get_tasks_file_path({ context: params.context })
...
export const delete_task_from_tree = (params: {
  tasks: Task[]
  timestamp: number
}): Task[] => {
  return params.tasks
...
export const find_task_in_tree_with_location = (params: {
  tasks: Task[]
  id: number
  ...
  for (let i = 0; i < params.tasks.length; i++) {
    if (params.tasks[i].created_at === params.id) {
      return { task: params.tasks[i], parent_id: effective_parent_id, index: i }
    }
...
export const insert_task_in_tree = (params: {
  tasks: Task[]
  task: Task
  parent_id: number | null
  index: number
...
export const mark_task_as_completed_if_matches_prompt = (params: {
  context: vscode.ExtensionContext
  prompt_text: string
}): Record<string, Task[]> | null => {
  if (!params.prompt_text.trim()) return null

  const all_tasks = load_all_tasks(params.context)
  ...
  if (file_changed) {
    save_all_tasks({ context: params.context, tasks: all_tasks })
