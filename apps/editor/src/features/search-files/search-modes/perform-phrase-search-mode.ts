...
import { t } from '@/i18n'
import { LAST_SEARCH_FILES_PHRASE_QUERY_STATE_KEY } from '@/constants/state-keys'
import { prompt_for_search_term } from '../utils/prompt-for-search-term'
import { search_files_by_phrase } from '../utils/search-files-by-phrase'
import { prompt_for_search_results } from '../utils/prompt-for-search-results'

export const perform_phrase_search_mode = async (params: {
...
> => {
  ...
  while (true) {
    const initial_search_term =
      local_queries[LAST_SEARCH_FILES_PHRASE_QUERY_STATE_KEY] !== undefined
        ? local_queries[LAST_SEARCH_FILES_PHRASE_QUERY_STATE_KEY]
    ...
    const result = await prompt_for_search_term(
      ...
      undefined,
      (value) => {
        local_queries[LAST_SEARCH_FILES_PHRASE_QUERY_STATE_KEY] = value
    ...
    local_queries[LAST_SEARCH_FILES_PHRASE_QUERY_STATE_KEY] = search_term_input
    ...
    const is_fast_search = files.length < 100

    if (is_fast_search) {
      matched_files = await search_files_by_phrase({
        files,
        search_term
      })
    } else {
      matched_files = await vscode.window.withProgress(
        ...
        async (progress, token) => {
          ...
          return await search_files_by_phrase({
            files,
            search_term,
            progress,
            token
