export const configurations = {
  'configurations.subtitle': {
    en: 'Manage configurations used for API calls.',
    pl: 'Zarządzaj konfiguracjami używanymi do zapytań API.',
    'zh-cn': '管理用于 API 调用的配置。',
    ja: 'API 呼び出しに使用する設定を管理します。',
    'zh-tw': '管理用於 API 呼叫的設定。',
    de: 'Verwalten Sie die für API-Aufrufe verwendeten Konfigurationen.',
    es: 'Gestionar las configuraciones utilizadas para las llamadas a la API.',
    fr: 'Gérer les configurations utilisées pour les appels API.',
    'pt-br': 'Gerenciar as configurações usadas para chamadas de API.',
    ru: 'Управление конфигурациями, используемыми для вызовов API.',
    ko: 'API 호출에 사용되는 구성을 관리합니다.',
    it: 'Gestisci le configurazioni utilizzate per le chiamate API.',
    tr: 'API çağrıları için kullanılan yapılandırmaları yönetin.',
    cs: 'Správa konfigurací používaných pro volání API.',
    hu: 'Az API hívásokhoz használt konfigurációk kezelése.',
    bg: 'Управление на конфигурациите, използвани за API извиквания.'
  },
  'configurations.notice': {
    en: 'Configurations define the provider, model and parameters for API calls.',
    pl: 'Konfiguracje określają dostawcę, model i parametry dla zapytań API.',
    ...
    'pt-br':
      'As configurações definem o provedor, o modelo e os parâmetros para as chamadas de API.',
