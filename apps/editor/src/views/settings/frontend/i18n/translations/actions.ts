export const actions = {
  ...
  'action.api-key-not-required': {
    en: 'API key not required',
    pl: 'Klucz API nie jest wymagany',
    'zh-cn': '不需要 API 密钥',
    ja: 'APIキーは不要です',
    'zh-tw': '不需要 API 金鑰',
    de: 'API-Schlüssel nicht erforderlich',
    es: 'Clave API no requerida',
    fr: 'Clé API non requise',
    'pt-br': 'Chave de API não necessária',
    ru: 'Ключ API не требуется',
    ko: 'API 키가 필요하지 않습니다',
    it: 'Chiave API non richiesta',
    tr: 'API anahtarı gerekli değil',
    cs: 'API klíč není vyžadován',
    hu: 'API kulcs nem szükséges',
    bg: 'API ключ не е задължителен'
  },
  'action.refresh-token': {
    ...
    es: 'Actualizar token',
    fr: 'Actualiser le jeton',
    'pt-br': 'Atualizar token',
    ru: 'Обновить токен',
    ko: '토큰 새로고침',
    it: 'Aggiorna token',
    tr: 'Belirteci yenile',
